...
export type Domain = 'skin' | 'dental' | 'facial';

export interface TreatmentInput {
  domain: Domain;
  treatment?: string; // Predefined identifier (id, sku, key, or name)
  name?: string;      // Custom name
  price?: number;     // Custom price
  sessions?: number;  // Only for skin
...
export function calculateUnifiedPlan(inputs: TreatmentInput[]): TreatmentPlanResult {
  ...
  for (const input of inputs) {
    ...
    // 2. Price Fetch
    if (isCustom) {
      if (!input.name) throw new Error('Custom treatments require a name.');
      ...
      if (!input.treatment) throw new Error('Predefined treatments require a treatment identifier.');
      
      let found = false;
      if (input.domain === 'skin') {
        const t = SKIN_TREATMENTS.find(t => t.sku === input.treatment || t.name === input.treatment || t.displayName === input.treatment);
        if (t) {
        ...
        const t = FACIAL_TREATMENTS.find(t => t.id.toString() === input.treatment || t.name === input.treatment);
        if (t) {
          pricePerSession = t.price;
          name = t.name;
          displayName = t.displayName;
        ...
        const tKey = Object.keys(DENTAL_TREATMENTS).find(k => k === input.treatment || DENTAL_TREATMENTS[k].name === input.treatment);
        if (tKey) {
          const t = DENTAL_TREATMENTS[tKey];
          pricePerSession = t.price;
          name = tKey; // Unique ID key
          displayName = t.name;
          category = t.category;
          if (t.protocol?.includes('scaling_free')) {
            needsScalingFree = true;
          }
          if (tKey === 'Scaling_Polishing') alreadyHasScaling = true;
          found = true;
        }
      }
    ...
    if (name === 'Scaling_Polishing' || name === 'Scaling & Polishing') alreadyHasScaling = true;
  ...
  if (!needsScalingFree) {
    ...
    needsScalingFree = breakdown.some(b => 
      b.domain === 'dental' && 
