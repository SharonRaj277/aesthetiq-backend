...
import { ScanResults, AISuggestion } from '../types/treatment';
...
const SUGGESTION_TOOL: Anthropic.Tool = {
  ...
  description:
    'Return structured, prioritised aesthetic treatment recommendations ' +
        ...
        items: {
          ...
          properties: {
            ...
            rationale: {
              type: 'string',
              description:
                'Concise clinical rationale (1–2 sentences) referencing the ' +
...
export async function generateAISuggestions(
  ...
  const scanLines = Object.entries(results)
    ...
    .map(([key, val]) => `  • ${humanise(key)}: ${val}/10 — ${severity(key, val as number)}`)
    .join('\n');
...
- Reference specific scan values in each rationale.`;
  ...
  return input.suggestions
    .map((s) => ({
      ...s,
      matchPercentage: parseFloat(Math.min(100, Math.max(0, s.matchPercentage)).toFixed(1)),
      estimatedSessions: Math.max(1, Math.round(s.estimatedSessions)),
      estimatedPricePerSession: parseFloat(s.estimatedPricePerSession.toFixed(2)),
    }))
...
function severity(key: string, val: number): string {
  // Hydration is inverted — low score = problem
  if (key === 'hydration') {
    if (val <= 2) return 'severely dry';
    if (val <= 4) return 'dry';
