...
class SessionService {
    ...
    async getSessionTracker(planId) {
        ...
        if (!tracker) {
            throw new types_1.HealthcareError('NOT_FOUND', `No session tracker found for plan ${planId}. Has the patient accepted the plan?`);
        }
        return tracker;
    }
    // ─────────────────────────────────────────
     ...
     */
    async completeSession(planId, treatment) {
        const tracker = await this.getSessionTracker(planId);
        const entry = tracker.sessions.find((s) => s.treatment.toLowerCase().trim() === treatment.toLowerCase().trim());
        if (!entry) {
            throw new types_1.HealthcareError('NOT_FOUND', `Treatment '${treatment}' not found in plan ${planId}. ` +
                `Available: [${tracker.sessions.map((s) => s.treatment).join(', ')}]`);
        }
        if (entry.completedSessions >= entry.totalSessions) {
            throw new types_1.HealthcareError('INVALID_STATE', `All ${entry.totalSessions} sessions for '${treatment}' are already complete.`);
        }
        // Increment
        entry.completedSessions += 1;
        const updatedSessions = tracker.sessions.map((s) => s.treatment.toLowerCase().trim() === treatment.toLowerCase().trim() ? entry : s);
        await this.storage.updateSessionTracker(planId, { sessions: updatedSessions });
        const updatedTracker = Object.assign(Object.assign({}, tracker), { sessions: updatedSessions, updatedAt: new Date() });
        const allComplete = updatedSessions.every((s) => s.completedSessions >= s.totalSessions);
    ...
    async getProgress(planId) {
        ...
        const treatments = tracker.sessions.map((s) => ({
            ...
            percentComplete: s.totalSessions > 0
